...
function prepMonth(m) { // Lui ve 1 nam
  cM=m-1;
  if (cM<0) { cM=11; cY--; }
  var nd=new Date(cY,cM,1);
  changeCal(nd);
...
function nextMonth(m) { // Tien ve 1 nam
  cM=m+1;
  if (cM>11) { cM=0; cY++;}
  var nd=new Date(cY,cM,1);
  changeCal(nd);
...
function changeCal(d) { // thay doi lich va tao nam moi
  var dd = 0;
...
function showCal(name) {  // Calendar
...
function clearDate() {      // reset ngay
  currentCal.form[currentCal.field].value="";
  hideCal();
}

function pickDate(d) {   // chon ngay
  hideCal();
  window.focus();
...
// user functions
function checkDate(name) {         // kiem tra ngay
...
function getCurrentDate() {    // tao ngay hien tai
...
function getNumbers(date) {  // tao so ngay
  var calRE = getFormat();
  var y, m, d;
